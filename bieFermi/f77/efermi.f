...
C
C     Modified by A. Marrazzo (antimo.marrazzo@epfl.ch) in 2016 and 2020
C     Bisection uses true min/max eigenvalues, changed tolerances
C
...
C
      IMPLICIT NONE
      integer nel, nbands, nsppts, ndim8, ndim10, ismear
      real*8 del, weight, occ, ef, eigval, sort
      real*8 eigmin, eigmax, enocc
      real*8 erf_c, fermid, delthm, spline, poshm, poshm2
...
C.....WARNINGS
      ...
      COMMON /WARN/ IWARN
      real*8 xacc
      PARAMETER ( JMAX =2000, XACC=1.0D-6) 
C      JMAX was 200, XACC was 1e-10
C.....WHICH SMEARING IS USED
      real*8 pi, ee, eesh, sq2i, piesqq, z, a, delcor, dx
      real*8 elow, en, f, fi, fmid, rtbis, test
      real*8 x, xe0, xe1, xe2, xmid, z1, zeta
      integer inel, isppt, j, j2, n, neig, nmax, nn
...
C     WRITE (9,*) 'del,xe1,xe2',del,XE1,XE2
      nmax=1
100   continue
...
140   CONTINUE
      ...
      IF(F*FMID .GE. 0.D0) THEN
        if (nmax.ge.100000) then !was 10'000
...
C      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      real*8 p1, q1, p2, q2, p3, q3, sqpi, x, x2, xi2
